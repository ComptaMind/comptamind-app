...
export default function SettingsPage() {
  ...
  return (
    <div>
      <Header
        title="Paramètres"
        subtitle="Gérez votre cabinet et les préférences ComptaMind"
        actions={
        ...
        <div className="flex-1 max-w-2xl">
          {tab === 'cabinet' && (
            <div className="card p-6">
              <h2 className="text-base font-bold text-slate-900 mb-5">Informations du cabinet</h2>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input className="input" placeholder="01 23 45 67 89" value={cabinetForm.tel} onChange={e => setCabinetForm({...cabinetForm, tel: e.target.value})} />
                  </div>
                </div>
                <div>
                  <label className="label">Adresse</label>
                  <input className="input" placeholder="15 rue de la Paix, 75001 Paris" value={cabinetForm.adresse} onChange={e => setCabinetForm({...cabinetForm, adresse: e.target.value})} />
                </div>
                <div>
              ...
              <div className="card p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Connectez votre compte Pennylane pour la saisie automatique</p>
                    </div>
                  </div>
                  <div className={`badge ${pennylaneConnected ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                    {pennylaneConnected ? '✓ Connecté' : 'Non connecté'}
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  <button onClick={() => setPennylaneConnected(true)} className="btn-secondary w-full">
                    <Link size={15} /> Tester et connecter
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="card p-6 border-brand-200 bg-brand-50/30">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h3 className="text-base font-bold text-slate-900">Plan Essentiel</h3>
                    <p className="text-sm text-slate-500">Actif · Renouvellement le 1er mai 2024</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-4 border-t border-brand-100">
                  {[
                    { label: 'Clients inclus', value: '20' },
                    { label: 'Opérations IA/mois', value: 'Illimitées' },
                    { label: 'Stockage', value: '50 Go' },
                  ].map((item, i) => (
              ...
              <div className="card p-6">
                <h3 className="text-sm font-bold text-slate-900 mb-4">Passer au plan Premium</h3>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { title: 'Essentiel', price: '99€/mois', clients: '20 clients', highlight: false },
                    { title: 'Premium', price: '199€/mois', clients: 'Clients illimités', highlight: true },
                  ].map((plan, i) => (
                    <div key={i} className={`p-4 rounded-xl border-2 ${plan.highlight ? 'border-brand-400 bg-brand-50' : 'border-slate-100'}`}>
                      ...
                      {plan.highlight && (
                        <button className="btn-primary w-full mt-3 text-sm py-2">Passer au Premium</button>
                      )}
                    </div>
                  ))}
                </div>
          ...
          {(tab === 'equipe' || tab === 'notifications') && (
            <div className="card p-10 text-center">
              ...
              <p className="text-slate-600 font-medium">Cette section arrive bientôt</p>
              <p className="text-slate-400 text-sm mt-1">La gestion d'équipe et les notifications seront disponibles dans la prochaine mise à jour.</p>
            </div>
          )}
        </div>
      </div>
